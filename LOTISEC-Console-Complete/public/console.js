...
function showLogin(message = "") {
  ...
  if (!overlay) {
    ...
    overlay.innerHTML = `<form class="console-login__card">
      <img src="assets/logo-lotisec.png" alt="LOTISEC">
      <small>ACCÈS INSTITUTIONNEL SÉCURISÉ</small>
      <h1>Console opérationnelle</h1>
      <p data-login-message>Connectez-vous avec votre compte professionnel.</p>
      <label>Téléphone<input name="phone" autocomplete="username" placeholder="+228 90 00 00 01" required></label>
    ...
    overlay.querySelectorAll("[data-quick-role]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        session = {
          ...
          user: {
            id: `demo-${role}`,
            phone: role === "admin" ? "+228 90 00 00 01" : role === "firefighter" ? "+228 91 18 00 01" : "+228 22 21 25 01",
            first_name: role === "admin" ? "Superviseur" : role === "firefighter" ? "Capitaine Secours" : "Dr. Responsable",
            last_name: role === "admin" ? "National" : role === "firefighter" ? "118 Lomé" : "CHU Tokoin",
            roles: [role === "hospital" ? "hospital_manager" : role],
            permissions: role === "admin" ? ["*"] : role === "firefighter" ? ["incidents:read", "incidents:manage", "interventions:manage"] : ["facilities:manage", "admissions:organization"],
            organization: { name: role === "hospital" ? "CHU Sylvanus Olympio" : role === "firefighter" ? "Sapeurs-Pompiers Lomé (118)" : "Direction Générale LOTISEC" }
          }
        };
...
function applyRbac() {
  ...
  const adminContainer = document.querySelector("[data-admin-view-container]");
  if (adminContainer) adminContainer.hidden = !isAdmin;

  let activePerspective = 'global';
  if (isAdmin) {
    activePerspective = currentAdminView;
  } else if (isHospital) {
    activePerspective = 'hospital';
  } else {
    activePerspective = 'firefighter';
  }
  ...
  document.querySelectorAll("[data-module]").forEach((el) => {
    el.hidden = !allowed.has(el.dataset.module);
  });
...
function renderHospitalCapacities() {
  ...
  return `
    ${moduleTop(moduleHeader("Capacités Hospitalières", "Gestion des Lits & Places d'Urgence", "Mise à jour en 1 clic des disponibilités d'accueil synchronisées avec la régulation du 118."), `<section class="module-metrics">
      ${metricCard("Places disponibles", totalBeds, "Réseau hospitalier", "green")}
      ${metricCard("CHU Sylvanus Olympio", hospitals[0]?.beds || 14, "Urgences 24h", "blue")}
      ${metricCard("CHU Campus", hospitals[1]?.beds || 8, "Urgences polyvalentes", "blue")}
      ${metricCard("Hôpital Dogta-Lafiè", hospitals[2]?.beds || 12, "Traumatologie", "orange")}
    </section>`)}
    <section class="hospital-grid">
      ${hospitals.map((h) => `
        <article class="resource-card hospital-card is-recommended">
          <header><span class="resource-icon resource-icon--hospital">H</span><div><small>${escapeHtml(h.phone)}</small><h2>${escapeHtml(h.name)}</h2></div><em class="resource-status resource-status--ok" id="bed-val-${h.id}">${h.beds} places libres</em></header>
          <div class="capacity-editor-box" style="margin-top:10px;">
            <strong>Ajuster les places d'urgences</strong>
            <small>${escapeHtml(h.specialty || 'Urgences')}</small>
            <div class="capacity-stepper">
              <button data-action="quick-bed" data-id="${h.id}" data-delta="-1">−</button>
              <span id="bed-count-${h.id}">${h.beds}</span>
              <button data-action="quick-bed" data-id="${h.id}" data-delta="1">＋</button>
            </div>
          </div>
          <div class="resource-facts" style="margin-top:10px;">
            <span><small>Occupation</small><strong>${h.occupancy}%</strong></span>
            <span><small>Services</small><strong>${(h.services||[]).slice(0,2).join(', ')}</strong></span>
          </div>
        </article>
      `).join('')}
    </section>
...
function renderAdmittedPatients() {
  ...
  return `
    ...
    <section class="module-card">
      <header><div><small>REGISTRE D'URGENCE</small><h2>Patients acheminés</h2></div></header>
...
function renderCurrentModule() {
  ...
  const renderers = {
    ...
    "Capacités & Lits": renderHospitalCapacities,
    "Fiches Patients": renderAdmittedPatients,
...
elements.dynamicPage?.addEventListener("click", async (event) => {
  ...
  if (action === "quick-bed") {
    const hosp = hospitals.find((h) => h.id === button.dataset.id);
    if (hosp) {
      const delta = parseInt(button.dataset.delta, 10);
      hosp.beds = Math.max(0, hosp.beds + delta);
      const spanCount = document.getElementById(`bed-count-${hosp.id}`);
      const spanVal = document.getElementById(`bed-val-${hosp.id}`);
      if (spanCount) spanCount.textContent = hosp.beds;
      if (spanVal) spanVal.textContent = `${hosp.beds} places libres`;
      syncHospitalCapacity(hosp);
      toast("Capacités synchronisées", `${hosp.name} : ${hosp.beds} place(s) disponible(s).`);
    }
  }
