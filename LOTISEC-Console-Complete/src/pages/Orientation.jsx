...
export default function Orientation({ alerts, ambulances, hospitals, mission, onConfirm, onNotify }) {
  ...
  const [sort, setSort] = useState('Recommandation opérationnelle')
  ...
  return <>
    ...
    <section className="surface mb-4 grid gap-3 p-3 md:grid-cols-[1fr_1fr_auto]">
      ...
      <select value={sort} onChange={event => setSort(event.target.value)} className="input"><option>Recommandation opérationnelle</option><option>Temps d’arrivée</option><option>Capacité disponible</option></select>
    ...
    <div className="lotisec-fixed-split grid gap-4 2xl:h-[calc(100vh-20rem)] 2xl:min-h-[540px] 2xl:grid-cols-[minmax(0,1fr)_430px]">
      ...
      <aside className="space-y-4">
        ...
        <section className="surface p-5">
          ...
          <div className="mt-4 grid grid-cols-3 gap-2 text-center"><Summary value={`${selected.decisionEta||selected.eta} min`} label="ETA"/><Summary value={selected.beds} label="Places"/><Summary value={selected.reception||'Ouverte'} label="Accueil"/></div>
...
function HealthCard({ hospital, recommended, selected, onSelect }) {
  ...
  const cardStyle=recommended?'border-emerald-300 bg-emerald-600 text-white shadow-lg shadow-emerald-900/20 ring-2 ring-emerald-300/40':selected?'border-emerald-500 bg-emerald-50/60 ring-2 ring-emerald-500/15 dark:bg-emerald-950/20':'border-slate-200 hover:border-emerald-300 hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-900'
...
function ComparisonTable({ candidates, selectedId, onSelect }) {
  return <section className="surface overflow-hidden">
    ...
    <div className="overflow-x-auto"><table className="w-full min-w-[820px] text-left text-sm"><thead className="bg-slate-50 text-xs text-slate-500 dark:bg-slate-900"><tr><th className="p-3">Centre de santé</th><th>Distance</th><th>ETA</th><th>Services</th><th>Capacité</th><th>Accueil</th></tr></thead><tbody>{candidates.map(hospital => <tr key={hospital.id} onClick={() => onSelect(hospital.id)} className={`cursor-pointer border-t border-slate-100 dark:border-slate-800 ${hospital.id === selectedId ? 'bg-emerald-50/60 dark:bg-emerald-950/20' : 'hover:bg-slate-50 dark:hover:bg-slate-900'}`}><td className="p-3 font-semibold">{hospital.name}</td><td>{hospital.distance} km</td><td><span className="inline-flex items-center gap-1"><Clock3 size={14}/>{hospital.decisionEta} min</span></td><td><div className="flex gap-1">{hospital.services?.slice(0, 3).map(service => <span key={service} title={service} className="grid h-7 w-7 place-items-center rounded-lg bg-blue-50 text-blue-600 dark:bg-blue-950/35"><Stethoscope size={14}/></span>)}</div></td><td><b className="text-emerald-600">{hospital.beds} places</b></td><td><Status tone={hospital.reception==='Ouverte'?'green':'amber'}>{hospital.reception}</Status></td></tr>)}</tbody></table></div>
  </section>
}
