...
export default function Statistics({alerts=[],ambulances=[],hospitals=[],mission,missionHistory=[],metrics=[],auditLog=[],fog={},mobileFeedStatus,dataMode='test',onNotify}){
  ...
  return <>
    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-[1fr_.9fr]">
      <section className="surface p-5"><div className="flex items-center justify-between"><div><h3 className="font-semibold">Répartition des incidents reçus</h3><p className="mt-1 text-xs muted">Calculée à partir du flux actuellement chargé dans la plateforme.</p></div><Status tone="blue">{alerts.length} TOTAL</Status></div><div className="mt-5 space-y-4">{typeCounts.length?typeCounts.map(([label,value])=><div key={label}><div className="mb-1.5 flex justify-between gap-3 text-sm"><span className="truncate">{label}</span><b>{value}</b></div><div className="h-2.5 rounded-full bg-slate-100 dark:bg-slate-800"><div className="h-2.5 rounded-full bg-blue-700 transition-all" style={{width:`${Math.max(8,value/maxType*100)}%`}}/></div></div>):<div className="rounded-xl bg-slate-50 p-6 text-center text-sm muted dark:bg-slate-900">Aucun incident reçu.</div>}</div></section>
