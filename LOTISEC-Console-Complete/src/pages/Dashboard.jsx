...
import { routes as demoRoutes } from '../data/demo'
...
export default function Dashboard({alerts,ambulances,hospitals,hospitalRanking,mission,onNavigate,onSimulateMobile,onOpenAlert,onStartDemo,fog}){
  ...
  const activeAlerts = alerts.filter(item=>!['Clôturée','Rejetée','Terminée'].includes(item.status))
  const criticalCount=activeAlerts.filter(item=>['Critique','Élevée','Haute'].includes(item.severity)).length

  // Interventions en cours calculées depuis les alertes actives et la mission réelle
  const ongoingInterventionsCount = (mission && mission.status !== 'Terminée' ? 1 : 0) + alerts.filter(a => [
    'Affectée','Ambulance affectée','En route','Ambulance en route','Sur place','Secours sur place',
    'Orientation hospitalière','Vers le centre de santé','Transfert hospitalier','Pris en charge','Victime prise en charge'
  ].includes(a.status) && a.id !== mission?.alertId).length

  const availableAmbulancesCount = ambulances.filter(a=>a.status==='Disponible').length
  ...
  return <>
    ...
    <div className="lotisec-sticky-kpis grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <Kpi label="Alertes actives" value={activeAlerts.length} icon={BellRing} tone="red" hint={`${criticalCount} à traiter en priorité · temps réel`}/>
      <Kpi label="Interventions en cours" value={ongoingInterventionsCount} icon={Clock3} tone="blue" hint={mission?`${mission.ambulanceId} · ${mission.status}`:`${ongoingInterventionsCount} mission(s) active(s)`}/>
      <Kpi label="Ambulances disponibles" value={availableAmbulancesCount} icon={Ambulance} tone="green" hint={`${availableAmbulancesCount} sur ${ambulances.length} unité(s)`}/>
    ...
    <div className="lotisec-fixed-split mt-4 grid gap-4 xl:h-[calc(100vh-16rem)] xl:min-h-[540px] xl:grid-cols-[minmax(0,1.55fr)_minmax(340px,.95fr)]">
      ...
      <div className="space-y-4">
        ...
        <section className="surface p-4"><SectionHeader title="Routage" link="Comparer" onClick={()=>onNavigate('routing')}/><div className="space-y-2">{demoRoutes.slice(0,2).map(r=><MiniRoute key={r.id} {...r} onClick={()=>onNavigate('routing')}/>)}</div></section>
      </div>
    </div>
