...
export const api = {
  ...
  updateIncidentStatus: (id, status) => request(`/api/v1/incidents/${id}/status`, { method:'PATCH', body:JSON.stringify({ status }) }),
  assignIncident: (id, payload) => request(`/api/v1/incidents/${id}/assignments`, { method:'POST', body:JSON.stringify(payload) }),
  interventions: () => request('/api/v1/interventions'),
  ambulances: () => request('/api/v1/resources'),
  resources: () => request('/api/v1/resources'),
  organizations: () => request('/api/v1/organizations'),
  hospitals: () => request('/api/v1/facilities'),
  facilities: () => request('/api/v1/facilities'),
